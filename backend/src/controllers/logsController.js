...
export async function createLog(req, res) {
  try {
    ...
    const tagsAIArray = tagsData.core_tags
      .split(",")
      .map((tag) => tag.trim())
...
export async function createLogWithContext(req, res) {
  try {
    ...
    const tagsAIArray = tagsData.core_tags
      ...
      .filter((tag) => tag.length > 0);
    const combinedArray = tagsAIArray.concat(tagsManuallyAdded);
    const logIDs = (similar_logIDS.similar_logs || []).filter((id) =>
...
export async function deleteLog(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Log deleted successfully", log: deletedLog });
  } catch (error) {
